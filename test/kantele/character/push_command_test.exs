defmodule Kantele.Character.PushCommandTest do
  use ExUnit.Case, async: true

  import Kalevala.ConnTest

  alias Kantele.Character.PushCommand
  ...
  describe "push 命令" do
    test "无参数时提示格式" do
      p = player()
      conn = PushCommand.run(build_conn(p), %{})
      assert output_text(conn) =~ "指令格式"
    end

    test "有参数时显示暂未开放" do
      p = player()
      conn = PushCommand.run(build_conn(p), %{"rest" => "李四 to north"})
      assert output_text(conn) =~ "暂未开放"
    end

    test "路由解析" do
      {:ok, parsed} = Kantele.Character.Commands.parse("push 李四 to north")
      assert parsed.module == PushCommand
    end
  end
end
