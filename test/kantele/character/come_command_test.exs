defmodule Kantele.Character.ComeCommandTest do
  use ExUnit.Case, async: true

  import Kalevala.ConnTest

  alias Kantele.Character.ComeCommand
  ...
  describe "come 命令" do
    test "无参数时提示" do
      p = player()
      conn = ComeCommand.run(build_conn(p), %{})
      assert output_text(conn) =~ "野兽跟随"
    end

    test "有参数时显示暂未开放" do
      p = player()
      conn = ComeCommand.run(build_conn(p), %{"target" => "狼"})
      assert output_text(conn) =~ "暂未开放"
    end

    test "路由解析" do
      {:ok, parsed} = Kantele.Character.Commands.parse("come 狼")
      assert parsed.module == ComeCommand
    end
  end
end
