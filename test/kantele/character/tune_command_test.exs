defmodule Kantele.Character.TuneCommandTest do
  use ExUnit.Case, async: true

  import Kalevala.ConnTest

  alias Kantele.Character.TuneCommand
  ...
  describe "tune 命令" do
    test "无频道时显示未收听" do
      p = player()
      conn = TuneCommand.run(build_conn(p), %{})
      assert output_text(conn) =~ "并没有收听任何频道"
    end

    test "路由解析" do
      {:ok, parsed} = Kantele.Character.Commands.parse("tune general")
      assert parsed.module == TuneCommand
    end
  end
end
