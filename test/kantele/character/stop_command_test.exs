defmodule Kantele.Character.StopCommandTest do
  use ExUnit.Case, async: true

  import Kalevala.ConnTest

  alias Kantele.Character.StopCommand
  ...
  describe "stop 命令" do
    test "无参数时提示" do
      p = player()
      conn = StopCommand.run(build_conn(p), %{})
      assert output_text(conn) =~ "野兽停止"
    end

    test "有参数时显示暂未开放" do
      p = player()
      conn = StopCommand.run(build_conn(p), %{"target" => "老虎"})
      assert output_text(conn) =~ "暂未开放"
    end

    test "路由解析" do
      {:ok, parsed} = Kantele.Character.Commands.parse("stop 老虎")
      assert parsed.module == StopCommand
    end
  end
end
