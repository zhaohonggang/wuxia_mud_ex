defmodule Kantele.Character.BjtimeCommandTest do
  use ExUnit.Case, async: true

  import Kalevala.ConnTest

  alias Kantele.Character.BjtimeCommand
  ...
  describe "bjtime 命令" do
    test "显示北京时间" do
      p = player()
      conn = BjtimeCommand.run(build_conn(p), %{})
      text = output_text(conn)
      assert text =~ "现在的时间是北京时间"
      assert text =~ ~r/\d{2}:\d{2}:\d{2}/
    end

    test "路由解析" do
      {:ok, parsed} = Kantele.Character.Commands.parse("bjtime")
      assert parsed.module == BjtimeCommand
    end
  end
end
