defmodule Kantele.Character.BeepCommandTest do
  use ExUnit.Case, async: true

  import Kalevala.ConnTest

  alias Kantele.Character.BeepCommand
  ...
  describe "beep 命令" do
    test "发送呼叫" do
      p = player()
      conn = BeepCommand.run(build_conn(p), %{"target" => "李四"})
      assert output_text(conn) =~ "李四"
      assert output_text(conn) =~ "呼叫"
    end

    test "路由解析" do
      {:ok, parsed} = Kantele.Character.Commands.parse("beep 李四")
      assert parsed.module == BeepCommand
    end
  end
end
