defmodule Kantele.Character.AcceptCommandTest do
  use ExUnit.Case, async: true

  import Kalevala.ConnTest

  alias Kantele.Character.AcceptCommand
  ...
  describe "accept 命令" do
    test "显示无挑战者" do
      p = player()
      conn = AcceptCommand.run(build_conn(p), %{})
      assert output_text(conn) =~ "没有人来挑战"
    end

    test "路由解析" do
      {:ok, parsed} = Kantele.Character.Commands.parse("accept")
      assert parsed.module == AcceptCommand
    end
  end
end
