defmodule Kantele.Character.ApprenticeCommandTest do
  use ExUnit.Case, async: true

  import Kalevala.ConnTest

  alias Kantele.Character.ApprenticeCommand
  ...
  describe "apprentice 命令" do
    test "发送family/apprentice事件" do
      p = player()
      conn = ApprenticeCommand.run(build_conn(p), %{"name" => "师父"})
      events = Enum.filter(conn.events, fn e -> e.topic == "family/apprentice" end)
    ...
    test "路由解析" do
      {:ok, parsed} = Kantele.Character.Commands.parse("apprentice 师父")
      assert parsed.module == ApprenticeCommand
    end
  end
end
