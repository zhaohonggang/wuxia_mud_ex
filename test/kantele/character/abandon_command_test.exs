defmodule Kantele.Character.AbandonCommandTest do
  use ExUnit.Case, async: true

  import Kalevala.ConnTest

  alias Kantele.Character.AbandonCommand
  ...
  describe "abandon 命令" do
    test "exp 太少时提示" do
      p = player(combat_exp: 500)
      conn = AbandonCommand.run(build_conn(p), %{"skill" => "exp"})
      assert output_text(conn) =~ "一无所知"
    end

    test "exp 足够时消耗经验" do
      p = player(combat_exp: 5000)
      conn = AbandonCommand.run(build_conn(p), %{"skill" => "exp"})
      assert output_text(conn) =~ "忘记武功"
    end

    test "未知技能时删除" do
      p = player()
      conn = AbandonCommand.run(build_conn(p), %{"skill" => "unknown_skill"})
      assert output_text(conn) =~ "好了"
    end

    test "路由解析" do
      {:ok, parsed} = Kantele.Character.Commands.parse("abandon sword")
      assert parsed.module == AbandonCommand
    end
  end
end
