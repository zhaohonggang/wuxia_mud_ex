defmodule Kantele.Character.EnableCommandTest do
  use ExUnit.Case, async: true

  import Kalevala.ConnTest

  alias Kantele.Character.EnableCommand
  ...
  describe "enable 命令" do
    test "无效技能时提示" do
      p = player()
      conn = EnableCommand.run(build_conn(p), %{"usage" => "sword", "skill" => "nonexistent"})
      assert output_text(conn) =~ "没有这项武功"
    end

    test "路由解析" do
      {:ok, parsed} = Kantele.Character.Commands.parse("enable sword liuxin-jian")
      assert parsed.module == EnableCommand
    end
  end
end
