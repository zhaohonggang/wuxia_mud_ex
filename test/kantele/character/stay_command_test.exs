defmodule Kantele.Character.StayCommandTest do
  use ExUnit.Case, async: true

  import Kalevala.ConnTest

  alias Kantele.Character.StayCommand
  ...
  describe "stay 命令" do
    test "显示暂未开放" do
      p = player()
      conn = StayCommand.run(build_conn(p), %{})
      assert output_text(conn) =~ "暂未开放"
    end

    test "路由解析" do
      {:ok, parsed} = Kantele.Character.Commands.parse("stay")
      assert parsed.module == StayCommand
    end
  end
end
