defmodule Kantele.Character.QuestAskCommandTest do
  use ExUnit.Case, async: true

  import Kalevala.ConnTest

  alias Kantele.Character.{AskQuestCommand, CancelQuestCommand}
  ...
  describe "ask_quest 命令" do
    test "发送 quest/ask 事件" do
      p = player()
      conn = AskQuestCommand.run(build_conn(p), %{"name" => "张三"})
      assert Enum.any?(conn.events, fn e -> e.topic == "quest/ask" end)
    end

    test "路由解析" do
      {:ok, parsed} = Kantele.Character.Commands.parse("ask_quest 张三")
      assert parsed.module == AskQuestCommand
    end
  end

  describe "cancel_quest 命令" do
    test "发送 quest/cancel 事件" do
      p = player()
      conn = CancelQuestCommand.run(build_conn(p), %{"name" => "张三"})
      assert Enum.any?(conn.events, fn e -> e.topic == "quest/cancel" end)
    end

    test "路由解析" do
      {:ok, parsed} = Kantele.Character.Commands.parse("cancel_quest 张三")
      assert parsed.module == AskQuestCommand
    end
  end
end
