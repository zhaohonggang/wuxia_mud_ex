defmodule Kantele.Character.BegCommandTest do
  use ExUnit.Case, async: true

  import Kalevala.ConnTest

  alias Kantele.Character.BegCommand
  ...
  describe "beg 命令" do
    test "无参数时提示格式" do
      p = player()
      conn = BegCommand.run(build_conn(p), %{})
      assert output_text(conn) =~ "指令格式"
    end

    test "有参数时显示暂未开放" do
      p = player()
      conn = BegCommand.run(build_conn(p), %{"rest" => "食物 from 李四"})
      assert output_text(conn) =~ "暂未开放"
    end

    test "路由解析" do
      {:ok, parsed} = Kantele.Character.Commands.parse("beg 食物 from 李四")
      assert parsed.module == BegCommand
    end
  end
end
