defmodule Kantele.Character.TellCommandTest do
  use ExUnit.Case, async: true

  import Kalevala.ConnTest

  alias Kantele.Character.TellCommand
  ...
  describe "tell 命令" do
    test "发送tell/send事件" do
      p = player()
      conn = TellCommand.run(build_conn(p), %{"name" => "李四", "text" => "消息"})
      events = Enum.filter(conn.events, fn e -> e.topic == "tell/send" end)
      assert length(events) == 1
      event = hd(events)
      assert event.data.name == "李四"
      assert event.data.text == "消息"
    end

    test "路由解析" do
      {:ok, parsed} = Kantele.Character.Commands.parse("tell 李四 消息")
      assert parsed.module == TellCommand
    end
  end
end
