defmodule Kantele.Character.IdCommandTest do
  use ExUnit.Case, async: true

  import Kalevala.ConnTest

  alias Kantele.Character.IdCommand
  ...
  describe "id 命令" do
    test "提示使用inventory" do
      p = player()
      conn = IdCommand.run(build_conn(p), %{})
      assert output_text(conn) =~ "inventory"
    end

    test "路由解析" do
      {:ok, parsed} = Kantele.Character.Commands.parse("id")
      assert parsed.module == IdCommand
    end
  end
end
