defmodule Kantele.Character.TrainCommandTest do
  use ExUnit.Case, async: true

  import Kalevala.ConnTest

  alias Kantele.Character.TrainCommand
  ...
  describe "train 命令" do
    test "无参数时提示" do
      p = player()
      conn = TrainCommand.run(build_conn(p), %{})
      assert output_text(conn) =~ "训练什么野兽"
    end

    test "有参数时显示暂未开放" do
      p = player()
      conn = TrainCommand.run(build_conn(p), %{"target" => "狼"})
      assert output_text(conn) =~ "暂未开放"
    end

    test "路由解析" do
      {:ok, parsed} = Kantele.Character.Commands.parse("train 狼")
      assert parsed.module == TrainCommand
    end
  end
end
