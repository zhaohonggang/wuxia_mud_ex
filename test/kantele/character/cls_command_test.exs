defmodule Kantele.Character.ClsCommandTest do
  use ExUnit.Case, async: true

  import Kalevala.ConnTest

  alias Kantele.Character.ClsCommand
  ...
  describe "cls 命令" do
    test "输出清屏转义序列" do
      p = player()
      conn = ClsCommand.run(build_conn(p), %{})
      assert output_text(conn) =~ "\e[2J\e[H"
    end

    test "路由解析" do
      {:ok, parsed} = Kantele.Character.Commands.parse("cls")
      assert parsed.module == ClsCommand
    end
  end
end
