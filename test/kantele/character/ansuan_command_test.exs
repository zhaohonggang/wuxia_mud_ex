defmodule Kantele.Character.AnsuanCommandTest do
  use ExUnit.Case, async: true

  import Kalevala.ConnTest

  alias Kantele.Character.AnsuanCommand
  ...
  describe "ansuan 命令" do
    test "无参数时显示暂未开放" do
      p = player()
      conn = AnsuanCommand.run(build_conn(p), %{})
      assert output_text(conn) =~ "暂未开放"
    end

    test "有参数时显示暂未开放" do
      p = player()
      conn = AnsuanCommand.run(build_conn(p), %{"rest" => "李四"})
      assert output_text(conn) =~ "暂未开放"
    end

    test "路由解析" do
      {:ok, parsed} = Kantele.Character.Commands.parse("ansuan 李四")
      assert parsed.module == AnsuanCommand
    end
  end
end
