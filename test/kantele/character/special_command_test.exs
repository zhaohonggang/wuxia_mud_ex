defmodule Kantele.Character.SpecialCommandTest do
  use ExUnit.Case, async: true

  import Kalevala.ConnTest

  alias Kantele.Character.SpecialCommand
  ...
  describe "special 命令" do
    test "无参数时显示暂未开放" do
      p = player()
      conn = SpecialCommand.run(build_conn(p), %{})
      assert output_text(conn) =~ "暂未开放"
    end

    test "有参数时显示暂未开放" do
      p = player()
      conn = SpecialCommand.run(build_conn(p), %{"skill" => "某种特技"})
      assert output_text(conn) =~ "暂未开放"
    end

    test "路由解析" do
      {:ok, parsed} = Kantele.Character.Commands.parse("special 某种特技")
      assert parsed.module == SpecialCommand
    end
  end
end
