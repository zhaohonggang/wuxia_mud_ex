defmodule Kantele.Character.WimpyCommandTest do
  ...
  describe "wimpy 命令" do
    test "无参数时显示当前设置" do
      p = player()
      conn = WimpyCommand.run(build_conn(p), %{})
      assert output_text(conn) =~ "气" or output_text(conn) =~ "没有设定"
    end

    test "无效值时提示错误" do
      p = player()
      conn = WimpyCommand.run(build_conn(p), %{"arg" => "100"})
      assert output_text(conn) =~ "指令格式"
    end

    test "路由解析" do
      {:ok, parsed} = Kantele.Character.Commands.parse("wimpy 50")
      assert parsed.module == WimpyCommand
    end
  end
end
