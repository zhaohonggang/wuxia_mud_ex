defmodule Kantele.Character.TopCommandTest do
  use ExUnit.Case, async: true

  import Kalevala.ConnTest

  alias Kantele.Character.TopCommand
  ...
  describe "top 命令" do
    test "无参数时显示帮助信息" do
      p = player()
      conn = TopCommand.run(build_conn(p), %{})
    ...
    test "exp参数显示暂未开放" do
      p = player()
      conn = TopCommand.run(build_conn(p), %{"category" => "exp"})
      text = output_text(conn)
      assert text =~ "暂未开放"
    end
    ...
    test "lv参数显示暂未开放" do
      p = player()
      conn = TopCommand.run(build_conn(p), %{"category" => "lv"})
      text = output_text(conn)
      assert text =~ "暂未开放"
    end

    test "league参数显示同盟排行" do
      p = player()
      conn = TopCommand.run(build_conn(p), %{"category" => "league"})
      text = output_text(conn)
      assert text =~ "结义同盟"
    end

    test "路由解析" do
      {:ok, parsed} = Kantele.Character.Commands.parse("top exp")
      assert parsed.module == TopCommand
    end
  end
end
