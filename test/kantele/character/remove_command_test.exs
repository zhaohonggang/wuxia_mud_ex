defmodule Kantele.Character.RemoveCommandTest do
  use ExUnit.Case, async: true

  import Kalevala.ConnTest

  alias Kantele.Character.RemoveCommand
  ...
  describe "remove 命令" do
    test "无参数时提示" do
      p = player()
      conn = RemoveCommand.run(build_conn(p), %{})
      assert output_text(conn) =~ "脱掉什么"
    end

    test "有参数时显示暂未开放" do
      p = player()
      conn = RemoveCommand.run(build_conn(p), %{"item" => "护甲"})
      assert output_text(conn) =~ "暂未开放"
    end

    test "路由解析" do
      {:ok, parsed} = Kantele.Character.Commands.parse("remove")
      assert parsed.module == RemoveCommand
    end
  end
end
