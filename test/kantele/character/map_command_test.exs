defmodule Kantele.Character.MapCommandTest do
  use ExUnit.Case, async: true

  import Kalevala.ConnTest

  alias Kantele.Character.MapCommand
  ...
  describe "map 命令" do
    test "发送zone-map/look事件" do
      p = player()
      conn = MapCommand.run(build_conn(p), %{})
      events = Enum.filter(conn.events, fn e -> e.topic == "zone-map/look" end)
      assert length(events) == 1
    end

    test "路由解析" do
      {:ok, parsed} = Kantele.Character.Commands.parse("map")
      assert parsed.module == MapCommand
    end
  end
end
