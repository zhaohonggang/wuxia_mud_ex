defmodule Kantele.Character.VoteCommandTest do
  use ExUnit.Case, async: true

  import Kalevala.ConnTest

  alias Kantele.Character.VoteCommand
  ...
  describe "vote 命令" do
    test "无参数时提示" do
      p = player()
      conn = VoteCommand.run(build_conn(p), %{})
      assert output_text(conn) =~ "要想清楚了"
    end

    test "有参数时显示暂未开放" do
      p = player()
      conn = VoteCommand.run(build_conn(p), %{"rest" => "李四"})
      assert output_text(conn) =~ "暂未开放"
    end

    test "路由解析" do
      {:ok, parsed} = Kantele.Character.Commands.parse("vote 李四")
      assert parsed.module == VoteCommand
    end
  end
end
