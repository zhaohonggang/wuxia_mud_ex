defmodule Kantele.Character.SystemCommandTest do
  use ExUnit.Case, async: true

  import Kalevala.ConnTest

  alias Kantele.Character.SystemCommand
  ...
  describe "system 命令" do
    test "显示系统资源信息" do
      p = player()
      conn = SystemCommand.run(build_conn(p), %{})
      text = output_text(conn)
      assert text =~ "游戏系统资源状态"
      assert text =~ "虚拟机内存使用"
      assert text =~ "总内存"
      assert text =~ "运行时间"
      assert text =~ "进程数"
    ...
    test "路由解析" do
      {:ok, parsed} = Kantele.Character.Commands.parse("system")
      assert parsed.module == SystemCommand
    end
  end
end
