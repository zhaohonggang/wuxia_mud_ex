defmodule Kantele.Character.GuardCommandTest do
  use ExUnit.Case, async: true

  import Kalevala.ConnTest

  alias Kantele.Character.GuardCommand
  ...
  describe "guard 命令" do
    test "guard时发送事件" do
      p = player()
      conn = GuardCommand.run(build_conn(p), %{"target" => "李四"})
      events = Enum.filter(conn.events, fn e -> e.topic == "guard/guard" end)
      assert length(events) == 1
    end

    test "路由解析" do
      {:ok, parsed} = Kantele.Character.Commands.parse("guard 李四")
      assert parsed.module == GuardCommand
    end
  end
end
