defmodule Kantele.Character.PasswdCommandTest do
  use ExUnit.Case, async: true

  import Kalevala.ConnTest

  alias Kantele.Character.PasswdCommand
  ...
  describe "passwd 命令" do
    test "无参数时提示格式" do
      p = player()
      conn = PasswdCommand.run(build_conn(p), %{})
      assert output_text(conn) =~ "指令格式"
    end

    test "密码太短时被拒绝" do
      p = player()
      conn = PasswdCommand.run(build_conn(p), %{"rest" => "1234"})
      assert output_text(conn) =~ "大于五位"
    end

    test "路由解析" do
      {:ok, parsed} = Kantele.Character.Commands.parse("passwd 新密码")
      assert parsed.module == PasswdCommand
    end
  end
end
