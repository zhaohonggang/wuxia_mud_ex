defmodule Kantele.Character.VersionCommandTest do
  use ExUnit.Case, async: true

  import Kalevala.ConnTest

  alias Kantele.Character.VersionCommand
  ...
  describe "version 命令" do
    test "显示版本信息" do
      p = player()
      conn = VersionCommand.run(build_conn(p), %{})
      text = output_text(conn)
      assert text =~ "Kalevala"
      assert text =~ "v0.1.0"
    end

    test "路由解析" do
      {:ok, parsed} = Kantele.Character.Commands.parse("version")
      assert parsed.module == VersionCommand
    end
  end
end
