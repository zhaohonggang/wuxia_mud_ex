defmodule Kantele.Character.DelayedCommandTest do
  use ExUnit.Case, async: true

  import Kalevala.ConnTest

  alias Kantele.Character.DelayedCommand
  ...
  describe "delayed 命令" do
    test "发送延迟事件" do
      p = player()
      conn = DelayedCommand.run(build_conn(p), %{"command" => "look"})
      assert Enum.any?(conn.events, fn e -> e.topic == "commands/delayed" end)
    end

    test "路由解析" do
      {:ok, parsed} = Kantele.Character.Commands.parse("delay look")
      assert parsed.module == DelayedCommand
    end
  end
end
