defmodule Kantele.Character.TalkCommandTest do
  use ExUnit.Case, async: true

  import Kalevala.ConnTest

  alias Kantele.Character.TalkCommand
  ...
  describe "talk 命令" do
    test "无参数时显示指令格式" do
      p = player()
      conn = TalkCommand.run(build_conn(p), %{})
    ...
    test "有参数时显示暂未开放" do
      p = player()
      conn = TalkCommand.run(build_conn(p), %{"rest" => "NPC 内容"})
      text = output_text(conn)
      assert text =~ "暂未开放"
    end

    test "路由解析" do
      {:ok, parsed} = Kantele.Character.Commands.parse("talk NPC")
      assert parsed.module == TalkCommand
    end
  end
end
