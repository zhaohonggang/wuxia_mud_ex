defmodule Kantele.Character.WhisperCommandTest do
  use ExUnit.Case, async: true

  import Kalevala.ConnTest

  alias Kantele.Character.WhisperCommand
  ...
  describe "whisper 命令" do
    test "发送whisper/send事件" do
      p = player()
      conn = WhisperCommand.run(build_conn(p), %{"name" => "李四", "text" => "悄悄话"})
      events = Enum.filter(conn.events, fn e -> e.topic == "whisper/send" end)
      assert length(events) == 1
      event = hd(events)
      assert event.data.name == "李四"
      assert event.data.text == "悄悄话"
    end

    test "路由解析" do
      {:ok, parsed} = Kantele.Character.Commands.parse("whisper 李四 悄悄话")
      assert parsed.module == WhisperCommand
    end
  end
end
