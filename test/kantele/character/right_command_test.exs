defmodule Kantele.Character.RightCommandTest do
  use ExUnit.Case, async: true

  import Kalevala.ConnTest

  alias Kantele.Character.RightCommand
  ...
  describe "right 命令" do
    test "无参数时提示" do
      p = player()
      conn = RightCommand.run(build_conn(p), %{})
      assert output_text(conn) =~ "答应谁"
    end

    test "空字符串参数时提示" do
      p = player()
      conn = RightCommand.run(build_conn(p), %{"arg" => "  "})
      assert output_text(conn) =~ "答应谁"
    end

    test "无待回复请求时提示" do
      p = player()
      conn = RightCommand.run(build_conn(p), %{"arg" => "李四"})
      assert output_text(conn) =~ "没有向你提出什么要求"
    end

    test "路由解析" do
      {:ok, parsed} = Kantele.Character.Commands.parse("right 李四")
      assert parsed.module == RightCommand
    end
  end
end
