defmodule Kantele.Character.QuitCommandTest do
  use ExUnit.Case, async: true

  import Kalevala.ConnTest

  alias Kantele.Character.QuitCommand
  ...
  describe "quit 命令" do
    test "显示goodbye并halt" do
      p = player()
      conn = QuitCommand.run(build_conn(p), %{})
      assert output_text(conn) =~ "Goodbye"
      assert conn.private.halt? == true
    end

    test "路由解析" do
      {:ok, parsed} = Kantele.Character.Commands.parse("quit")
      assert parsed.module == QuitCommand
    end
  end
end
