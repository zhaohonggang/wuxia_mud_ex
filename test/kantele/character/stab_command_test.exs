defmodule Kantele.Character.StabCommandTest do
  use ExUnit.Case, async: true

  import Kalevala.ConnTest

  alias Kantele.Character.StabCommand
  ...
  describe "stab 命令" do
    test "无参数时提示" do
      p = player()
      conn = StabCommand.run(build_conn(p), %{})
      assert output_text(conn) =~ "用什么物品"
    end

    test "有参数时显示暂未开放" do
      p = player()
      conn = StabCommand.run(build_conn(p), %{"item" => "旗子"})
      assert output_text(conn) =~ "暂未开放"
    end

    test "路由解析" do
      {:ok, parsed} = Kantele.Character.Commands.parse("stab 旗子")
      assert parsed.module == StabCommand
    end
  end
end
