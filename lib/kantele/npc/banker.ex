defmodule Kantele.Npc.Banker do
  ...
  defp convert_ok(money_map, amount, from, to, from_base, to_base) do
    # 从大面额换小面额时，不够整份的舍去
    amount = if from_base < to_base, do: amount - rem(amount, div(to_base, from_base)), else: amount
    if amount == 0, do: {:error, "不够换"}, else: do_convert(money_map, amount, from, to, from_base, to_base)
  end
  ...
  def deposit(money_map, balance, amount, what) do
    ...
    cond do
      Map.get(money_map, what, 0) < amount -> {:error, "带的#{what}不够"}
      amount < 1 -> {:error, "你想存多少？"}
      true ->
  ...
  def withdraw(money_map, balance, amount, what) do
    base = Money.denominations()[what]
    cond do
      amount < 1 -> {:error, "你想取多少钱？"}
      amount >= @max_withdraw -> {:error, "这么大数目本店没这么多零散现金"}
      is_nil(base) -> {:error, "你想取出什么钱？"}
      amount * base > (balance || 0) -> {:error, "你存的钱不够取"}
      true ->
