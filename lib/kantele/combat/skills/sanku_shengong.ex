...
defmodule Kantele.Combat.Skills.SankuShengong.Dispel do
  ...
  defp gate_self_or_target(_ctx), do: true

  defp gate_neili(ctx) do
    cost = if ctx.target == ctx.character, do: 100, else: 250
    ctx.character.meta.vitals.neili >= cost
  end

  defp gate_not_fighting_target(ctx) do
    ctx.target == ctx.character || not ctx.target.meta.combat.busy > 0 ||
...
defmodule Kantele.Combat.Skills.SankuShengong.Roar do
  ...
  def run(conn) do
    ...
    with :ok <- gate_no_fight(ctx(character)),
      ...
      room_chars = Map.get(conn.private, :room_characters, [])
      Enum.each(room_chars, fn target ->
        if target != character and target.meta.vitals.alive? do
  ...
  def resolve_incoming(conn, character, attacker, data) do
    ...
    if div(skill, 2) + rng.(div(skill, 2)) < con * 2 do
      ...
      if damage > 0 do
        ...
        new_eff_jing = max(character.meta.vitals.eff_jing - damage, 0)
        ...
        new_character =
          if new_jing < 1 or new_eff_jing < 1 do
            %{character | meta: %{character.meta | vitals: %{character.meta.vitals | jing: 1, eff_jing: 1, unconscious: true}}}
          else
            %{character | meta: %{character.meta | vitals: %{character.meta.vitals | jing: new_jing, eff_jing: new_eff_jing}}}
          end
