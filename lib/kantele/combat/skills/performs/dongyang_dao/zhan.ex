defmodule Kantele.Combat.Skills.Performs.DongyangDao.Zhan do
  @moduledoc """
  斩「zhan」（对照 `kungfu/skill/dongyang-dao/zhan.c`）

  东洋刀法攻击绝招：消耗 300 内力，发起 `combat/perform-incoming` 事件，
  由目标侧 `resolve_incoming` 判定命中并施加伤害。

  差异（TODO(migrate)）：
  - LPC 仅 `level:dongyang-dao>=70,force>=120`；本版加 `mapped` 与 `neili` 门槛；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "dongyang-dao")

    if level < 70 do
      {:error, "你的东洋刀法不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "blade") == "dongyang-dao" do
      :ok
    else
      {:error, "你没有激发东洋刀法，无法施展#{@move_name}。\n"}
    end
  end
