defmodule Kantele.Combat.Skills.Performs.KunlunQifa.Liu do
  @moduledoc """
  流星电闪「liu」（对照 `kungfu/skill/kunlun-qifa/liu.c`）

  昆仑气法单体攻击：消耗 150 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:kunlun-qifa>=120,force>=200`；本版加 `mapped` 与 `neili` 门槛；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "kunlun-qifa")

    if level < 120 do
      {:error, "你的昆仑气法不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "force") == "kunlun-qifa" do
      :ok
    else
      {:error, "你没有激发昆仑气法，无法施展#{@move_name}。\n"}
    end
  end
