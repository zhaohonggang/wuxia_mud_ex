defmodule Kantele.Combat.Skills.Performs.TaijiJian.Zhuan do
  @moduledoc """
  剑轮岚转「zhuan」（对照 `kungfu/skill/taiji-jian/zhuan.c`）

  太极剑法单体攻击：消耗 250 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:taiji-jian>=160`；本版加 `mapped` 与 `neili` 门槛；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "taiji-jian")

    if level < 160 do
      {:error, "你的太极剑法不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "sword") == "taiji-jian" do
      :ok
    else
      {:error, "你没有激发太极剑法，无法施展#{@move_name}。\n"}
    end
  end
