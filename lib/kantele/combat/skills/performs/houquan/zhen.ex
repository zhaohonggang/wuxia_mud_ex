defmodule Kantele.Combat.Skills.Performs.Houquan.Zhen do
  @moduledoc """
  猴拳「zhen」（对照 `kungfu/skill/houquan/zhen.c`）

  猴拳单体攻击：消耗 100 内力，对战斗中对手出招（空手/拳脚）。

  差异（TODO(migrate)）：
  - LPC 仅 `level:houquan>=50`；本版加 `mapped` 与 `neili` 门槛；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "houquan")

    if level < 50 do
      {:error, "你的猴拳不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "cuff") == "houquan" do
      :ok
    else
      {:error, "你没有激发猴拳，无法施展#{@move_name}。\n"}
    end
  end
