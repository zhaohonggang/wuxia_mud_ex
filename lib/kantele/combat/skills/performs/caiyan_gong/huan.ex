defmodule Kantele.Combat.Skills.Performs.CaiyanGong.Huan do
  @moduledoc """
  云霞幻生「huan」（对照 `kungfu/skill/caiyan-gong/huan.c`）

  彩云功攻击绝招：消耗 200 内力，发起 `combat/perform-incoming` 事件。

  差异（TODO(migrate)）：
  - LPC 仅 `level:caiyan-gong>=60,force>=200`；本版加 `mapped` 与 `neili` 门槛；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "caiyan-gong")

    if level < 60 do
      {:error, "你的彩云功不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "club") == "caiyan-gong" do
      :ok
    else
      {:error, "你没有激发彩云功，无法施展#{@move_name}。\n"}
    end
  end
