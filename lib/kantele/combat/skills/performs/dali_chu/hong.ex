defmodule Kantele.Combat.Skills.Performs.DaliChu.Hong do
  @moduledoc """
  轰雷万钧「hong」（对照 `kungfu/skill/dali-chu/hong.c`）

  大力锄法攻击绝招：消耗 200 内力，发起 `combat/perform-incoming` 事件。

  差异（TODO(migrate)）：
  - LPC 仅 `level:dali-chu>=80,force>=120`；本版加 `mapped` 与 `neili` 门槛；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "dali-chu")

    if level < 80 do
      {:error, "你的大力锄法不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "hammer") == "dali-chu" do
      :ok
    else
      {:error, "你没有激发大力锄法，无法施展#{@move_name}。\n"}
    end
  end
  ...
  defp apply_perform(conn, character, target, level) do
    ...
    conn =
      Broadcast.publish(
        conn,
        "$N锄光如雷，使出大力锄法「轰雷万钧」，锄势如万钧雷霆般砸向$n！\n",
        n1: character.name,
        n2: target.name
      )
