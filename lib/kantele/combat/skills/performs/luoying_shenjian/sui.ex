defmodule Kantele.Combat.Skills.Performs.LuoyingShenjian.Sui do
  @moduledoc """
  随波逐流「sui」（对照 `kungfu/skill/luoying-shenjian/sui.c`）

  落英神剑单体攻击：消耗 100 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:luoying-shenjian>=80,force>=100`；本版加 `mapped` 与 `neili` 门槛；
  ...
  @perform_id "luoying-shenjian/sui"
  @move_name "「随波逐流」"
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "luoying-shenjian")

    if level < 80 do
      {:error, "你的落英神剑不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "sword") == "luoying-shenjian" do
      :ok
    else
      {:error, "你没有激发落英神剑，无法施展#{@move_name}。\n"}
    end
  end
  ...
  defp apply_perform(conn, character, target, level) do
    ...
    conn =
      Broadcast.publish(
        conn,
        "$N剑光如落英缤纷，使出落英神剑「随波逐流」，剑势绵绵不绝刺向$n！\n",
        n1: character.name,
        n2: target.name
      )
