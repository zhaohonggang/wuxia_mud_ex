defmodule Kantele.Combat.Skills.Performs.ShunvJian.Ning do
  @moduledoc """
  宁氏一剑「ning」（对照 `kungfu/skill/shunv-jian/ning.c`）

  淑女剑法单体攻击：消耗 250 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:shunv-jian>=100,force>=100,dodge>=100`；本版加 `mapped` 与 `neili` 门槛；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "shunv-jian")

    if level < 100 do
      {:error, "你的淑女剑法不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "sword") == "shunv-jian" do
      :ok
    else
      {:error, "你没有激发淑女剑法，无法施展#{@move_name}。\n"}
    end
  end
