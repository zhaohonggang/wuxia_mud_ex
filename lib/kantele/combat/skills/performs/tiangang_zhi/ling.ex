defmodule Kantele.Combat.Skills.Performs.TiangangZhi.Ling do
  @moduledoc """
  凌空指穴「ling」（对照 `kungfu/skill/tiangang-zhi/ling.c`）

  天罡指单体攻击：消耗 100 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:tiangang-zhi>=100,force>=150`；本版加 `mapped` 与 `neili` 门槛；
  ...
  defp check_weapon(combat) do
    ...
    if weapon && Map.get(weapon, :skill_type) == "finger" do
      :ok
    else
      {:error, "你使用的武器不对。\n"}
    end
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "tiangang-zhi")

    if level < 100 do
      {:error, "你的天罡指不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "finger") == "tiangang-zhi" do
      :ok
    else
      {:error, "你没有激发天罡指，无法施展#{@move_name}。\n"}
    end
  end
  ...
  def resolve_incoming(conn, character, attacker, data) do
    ...
    bindings = [n1: attacker.name, n2: character.name, weapon2: weapon_name || "手指"]
