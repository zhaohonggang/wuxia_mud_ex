defmodule Kantele.Combat.Skills.Performs.SanwuShou.Wei do
  @moduledoc """
  无所不为「wei」（对照 `kungfu/skill/sanwu-shou/wei.c`）

  散手单体攻击：消耗 100 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:sanwu-shou>=70`；本版加 `mapped` 与 `neili` 门槛；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "sanwu-shou")

    if level < 70 do
      {:error, "你的散手不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "whip") == "sanwu-shou" do
      :ok
    else
      {:error, "你没有激发散手，无法施展#{@move_name}。\n"}
    end
  end
