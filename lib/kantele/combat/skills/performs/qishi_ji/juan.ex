defmodule Kantele.Combat.Skills.Performs.QishiJi.Juan do
  @moduledoc """
  旋转卷刺「juan」（对照 `kungfu/skill/qishi-ji/juan.c`）

  齐氏戟法单体攻击：消耗 200 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:qishi-ji>=100,force>=150`；本版加 `mapped` 与 `neili` 门槛；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "qishi-ji")

    if level < 100 do
      {:error, "你的齐氏戟法不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "club") == "qishi-ji" do
      :ok
    else
      {:error, "你没有激发齐氏戟法，无法施展#{@move_name}。\n"}
    end
  end
  ...
  defp apply_perform(conn, character, target, level) do
    ...
    conn =
      Broadcast.publish(
        conn,
        "$N手中戟影旋转，使出齐氏戟法「旋转卷刺」，戟尖如龙卷风般刺向$n！\n",
        n1: character.name,
        n2: target.name
      )
