defmodule Kantele.Combat.Skills.Performs.MizongHouquan.Zhou do
  @moduledoc """
  迷踪猴拳「zhou」（对照 `kungfu/skill/mizong-houquan/zhou.c`）

  迷踪猴拳单体攻击：消耗 500 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:mizong-houquan>=200`；本版加 `mapped` 与 `neili` 门槛；
  ...
  @perform_id "mizong-houquan/zhou"
  @move_name "「zhou」"
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "mizong-houquan")

    if level < 200 do
      {:error, "你的迷踪猴拳不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "cuff") == "mizong-houquan" do
      :ok
    else
      {:error, "你没有激发迷踪猴拳，无法施展#{@move_name}。\n"}
    end
  end
