defmodule Kantele.Combat.Skills.Performs.DachengFosuo.Chanrao do
  @moduledoc """
  缠绕「chanrao」（对照 `kungfu/skill/dacheng-fosuo/chanrao.c`）

  大乘佛索单体攻击：消耗 200 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:dacheng-fosuo>=80`；本版加 `mapped` 与 `neili` 门槛；
  ...
  @perform_id "dacheng-fosuo/chanrao"
  @move_name "「缠绕」"
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "dacheng-fosuo")

    if level < 80 do
      {:error, "你的大乘佛索不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "whip") == "dacheng-fosuo" do
      :ok
    else
      {:error, "你没有激发大乘佛索，无法施展#{@move_name}。\n"}
    end
  end
