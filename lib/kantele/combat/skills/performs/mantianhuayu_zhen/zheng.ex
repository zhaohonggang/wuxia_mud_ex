defmodule Kantele.Combat.Skills.Performs.MantianhuayuZhen.Zheng do
  @moduledoc """
  整「zheng」（对照 `kungfu/skill/mantianhuayu-zhen/zheng.c`）

  满天花雨阵单体攻击：消耗 150 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:force>=150,mantianhuayu-zhen>=100`；本版加 `mapped` 与 `neili` 门槛；
  ...
  @perform_id "mantianhuayu-zhen/zheng"
  @move_name "「整」"
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "mantianhuayu-zhen")

    if level < 100 do
      {:error, "你的满天花雨阵不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "force") == "mantianhuayu-zhen" do
      :ok
    else
      {:error, "你没有激发满天花雨阵，无法施展#{@move_name}。\n"}
    end
  end
  ...
  def resolve_incoming(conn, character, attacker, data) do
    ...
    bindings = [n1: attacker.name, n2: character.name, weapon2: weapon_name || "气劲"]
