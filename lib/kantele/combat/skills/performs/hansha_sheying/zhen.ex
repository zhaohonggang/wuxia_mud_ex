defmodule Kantele.Combat.Skills.Performs.HanshaSheying.Zhen do
  @moduledoc """
  无影针「zhen」（对照 `kungfu/skill/hansha-sheying/zhen.c`）

  寒沙射影单体攻击：消耗 150 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:hansha-sheying>=120,force>=150`；本版加 `mapped` 与 `neili` 门槛；
  ...
  @perform_id "hansha-sheying/zhen"
  @move_name "「无影针」"
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "hansha-sheying")

    if level < 120 do
      {:error, "你的寒沙射影不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "throwing") == "hansha-sheying" do
      :ok
    else
      {:error, "你没有激发寒沙射影，无法施展#{@move_name}。\n"}
    end
  end
