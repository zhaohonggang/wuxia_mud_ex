defmodule Kantele.Combat.Skills.Performs.SuqinBeijian.Can do
  @moduledoc """
  残阳劲「can」（对照 `kungfu/skill/suqin-beijian/can.c`）

  苏秦背剑单体攻击：消耗 50 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:suqin-beijian>=60,force>=80`；本版加 `mapped` 与 `neili` 门槛；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "suqin-beijian")

    if level < 60 do
      {:error, "你的苏秦背剑不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "sword") == "suqin-beijian" do
      :ok
    else
      {:error, "你没有激发苏秦背剑，无法施展#{@move_name}。\n"}
    end
  end
