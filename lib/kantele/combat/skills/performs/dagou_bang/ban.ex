defmodule Kantele.Combat.Skills.Performs.DagouBang.Ban do
  @moduledoc """
  绊字诀「ban」（对照 `kungfu/skill/dagou-bang/ban.c`）

  打狗棒法攻击绝招：消耗 300 内力，发起 `combat/perform-incoming` 事件。

  差异（TODO(migrate)）：
  - LPC 仅 `level:force>=240`；本版加 `mapped` 与 `neili` 门槛；
  - 命中判定：`rand(level) > parry/2`，忙乱 `level/22+2` 轮。
  - LPC 有 `set:eff_jing=1,eff_qi=1`，本版简化处理。
  """
