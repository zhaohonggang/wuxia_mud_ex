defmodule Kantele.Combat.Skills.Performs.DaojianGuizhen.Daojian do
  @moduledoc """
  刀剑七重天「daojian」（对照 `kungfu/skill/daojian-guizhen/daojian.c`）

  刀剑归真攻击绝招：消耗 500 内力，发起 `combat/perform-incoming` 事件，
  由目标侧 `resolve_incoming` 判定命中并施加伤害。

  差异（TODO(migrate)）：
  - LPC 仅 `level:daojian-guizhen>=200,daojian-guizhen>=250`；本版加 `mapped` 与 `neili` 门槛；
  - 命中判定：`rand(level) > parry/2`，忙乱 `level/22+2` 轮。
  - 伤害计算：`damage = level * 2`（简化），LPC 有复杂公式。
  - LPC 有 `temp:handing`，暂未建模。
  """
  ...
  @perform_id "daojian-guizhen/daojian"
  @move_name "「刀剑七重天」"
  ...
  defp check_weapon(combat) do
    ...
    if weapon && (Map.get(weapon, :skill_type) == "blade" || Map.get(weapon, :skill_type) == "sword") do
      :ok
    else
      {:error, "你使用的武器不对。\n"}
    end
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "daojian-guizhen")

    if level < 200 do
      {:error, "你的刀剑归真不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    mapped_blade = Stats.mapped(stats, "blade")
    mapped_sword = Stats.mapped(stats, "sword")

    if mapped_blade == "daojian-guizhen" or mapped_sword == "daojian-guizhen" do
      :ok
    else
      {:error, "你没有激发刀剑归真，无法施展#{@move_name}。\n"}
    end
  end
