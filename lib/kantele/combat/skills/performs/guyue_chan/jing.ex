defmodule Kantele.Combat.Skills.Performs.GuyueChan.Jing do
  @moduledoc """
  皓月惊空「jing」（对照 `kungfu/skill/guyue-chan/jing.c`）

  孤月铲法攻击绝招：消耗 200 内力，发起 `combat/perform-incoming` 事件，
  由目标侧 `resolve_incoming` 判定命中并施加伤害。

  差异（TODO(migrate)）：
  - LPC 仅 `level:guyue-chan>=80,force>=100`；本版加 `mapped` 与 `neili` 门槛；
  - 命中判定：`rand(level) > parry/2`，忙乱 `level/22+2` 轮。
  - 伤害计算：`damage = level * 2`（简化），LPC 为 `ap/2+rand(ap/2)+rand(damage/3)`。
  """
  ...
  defp check_weapon(combat) do
    ...
    if weapon && Map.get(weapon, :skill_type) == "staff" do
      ...
      {:error, "你所使用的武器不对，难以施展#{@move_name}。\n"}
    end
  end

  defp check_skill_level(stats) do
    level = Stats.skill(stats, "guyue-chan")

    if level < 80 do
      {:error, "你孤月铲法不够娴熟，难以施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "staff") == "guyue-chan" do
      :ok
    else
      {:error, "你没有激发孤月铲法，难以施展#{@move_name}。\n"}
    end
  end

  defp check_neili(character) do
    if character.meta.vitals.neili < 200 do
      {:error, "你现在的真气不足，难以施展#{@move_name}。\n"}
    else
      :ok
    end
  end
  ...
  defp apply_perform(conn, character, target, level) do
    ...
    conn =
      Broadcast.publish(
        conn,
        "$N怒喝一声，将手中的兵刃挥舞得呼呼做响，施展#{@move_name}猛然砸向$n！\n",
        n1: character.name,
        n2: target.name
      )
    ...
    if hit do
      ...
      |> Broadcast.publish(
        Messages.interpolate(
          "结果$p想闪避已不及，被$N的#{@move_name}打中胸口，喷出一口鲜血，受到#{damage}点伤害！\n",
          bindings
        )
      )
