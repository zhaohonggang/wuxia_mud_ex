defmodule Kantele.Combat.Skills.Performs.QixingJian.Xing do
  @moduledoc """
  群星璀璨「xing」（对照 `kungfu/skill/qixing-jian/xing.c`）

  七星剑法单体攻击：消耗 50 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:qixing-jian>=60,force>=80`；本版加 `mapped` 与 `neili` 门槛；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "qixing-jian")

    if level < 60 do
      {:error, "你的七星剑法不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "sword") == "qixing-jian" do
      :ok
    else
      {:error, "你没有激发七星剑法，无法施展#{@move_name}。\n"}
    end
  end
