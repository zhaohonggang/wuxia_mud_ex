defmodule Kantele.Combat.Skills.Performs.JiuquJian.Wan do
  @moduledoc """
  九曲十八弯「wan」（对照 `kungfu/skill/jiuqu-jian/wan.c`）

  九曲剑法单体攻击：消耗 300 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:jiuqu-jian>=100`；本版加 `mapped` 与 `neili` 门槛；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "jiuqu-jian")

    if level < 100 do
      {:error, "你的九曲剑法不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "sword") == "jiuqu-jian" do
      :ok
    else
      {:error, "你没有激发九曲剑法，无法施展#{@move_name}。\n"}
    end
  end
  ...
  defp apply_perform(conn, character, target, level) do
    ...
    conn =
      Broadcast.publish(
        conn,
        "$N剑势九曲十八弯，使出九曲剑法「九曲十八弯」，剑势迂回缠绕刺向$n！\n",
        n1: character.name,
        n2: target.name
      )
