defmodule Kantele.Combat.Skills.Performs.HeixueShenzhen.Luo do
  @moduledoc """
  星罗棋布「luo」（对照 `kungfu/skill/heixue-shenzhen/luo.c`）

  黑血神针单体攻击：消耗 150 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:heixue-shenzhen>=140,force>=200`；本版加 `mapped` 与 `neili` 门槛；
  ...
  @perform_id "heixue-shenzhen/luo"
  @move_name "「星罗棋布」"
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "heixue-shenzhen")

    if level < 140 do
      {:error, "你的黑血神针不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "throwing") == "heixue-shenzhen" do
      :ok
    else
      {:error, "你没有激发黑血神针，无法施展#{@move_name}。\n"}
    end
  end
  ...
  defp apply_perform(conn, character, target, level) do
    ...
    conn =
      Broadcast.publish(
        conn,
        "$N双手齐扬，使出黑血神针「星罗棋布」，针雨如星罗棋布般罩向$n！\n",
        n1: character.name,
        n2: target.name
      )
