defmodule Kantele.Combat.Skills.Performs.DandingDao.Kou do
  @moduledoc """
  环环相扣「kou」（对照 `kungfu/skill/danding-dao/kou.c`）

  澹定刀法攻击绝招：消耗 150 内力，发起 `combat/perform-incoming` 事件。

  差异（TODO(migrate)）：
  - LPC 仅 `level:danding-dao>=80,force>=100`；本版加 `mapped` 与 `neili` 门槛；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "danding-dao")

    if level < 80 do
      {:error, "你的澹定刀法不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "blade") == "danding-dao" do
      :ok
    else
      {:error, "你没有激发澹定刀法，无法施展#{@move_name}。\n"}
    end
  end
  ...
  defp apply_perform(conn, character, target, level) do
    ...
    conn =
      Broadcast.publish(
        conn,
        "$N刀法环环相扣，使出澹定刀法「环环相扣」，刀圈层层叠叠罩向$n！\n",
        n1: character.name,
        n2: target.name
      )
