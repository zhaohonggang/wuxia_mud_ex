defmodule Kantele.Combat.Skills.Performs.FeixingShu.Hua do
  @moduledoc """
  满天花雨「hua」（对照 `kungfu/skill/feixing-shu/hua.c`）

  飞星术单体攻击：消耗 250 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:feixing-shu>=100,force>=120`；本版加 `mapped` 与 `neili` 门槛；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "feixing-shu")

    if level < 100 do
      {:error, "你的飞星术不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "throwing") == "feixing-shu" do
      :ok
    else
      {:error, "你没有激发飞星术，无法施展#{@move_name}。\n"}
    end
  end
