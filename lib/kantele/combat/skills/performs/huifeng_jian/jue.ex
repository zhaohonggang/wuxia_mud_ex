defmodule Kantele.Combat.Skills.Performs.HuifengJian.Jue do
  @moduledoc """
  绝剑「jue」（对照 `kungfu/skill/huifeng-jian/jue.c`）

  回风拂柳剑法单体攻击：消耗 400~500 内力，五连击，发起
  `combat/perform-incoming` 事件，由目标侧 `resolve_incoming` 判定命中并施加伤害。

  差异（TODO(migrate)）：
  - LPC 另要求 force>=200；本版仅以 huifeng-jian>=120 与 mapped 把关；
  - LPC 五招分别对 str/int/con/dex/sword 对敌四项属性与 parry 判定，
    本版合为一次 parry 判定；
  - 命中判定：`rand(level) > parry/2`，忙乱 `level/22+2` 轮。
  - 伤害计算：`damage = level * 2`（简化），LPC 为 `huifeng-jian/2+rand/2`。
  """
  ...
  defp check_skill_level(stats) do
    ...
    if level < 120 do
      {:error, "你回风拂柳剑法不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp apply_perform(conn, character, target, level) do
    ...
    conn =
      Broadcast.publish(
        conn,
        "$N一声暴喝，手中的剑对准$n直劈而下，施展#{@move_name}绝技！\n",
        n1: character.name,
        n2: target.name
      )
    ...
    if hit do
      ...
      |> Broadcast.publish(
        Messages.interpolate(
          "结果$p奋力抵挡，却哪里招架得住，被$N这一剑劈下，登时鲜血迸流，受到#{damage}点伤害！\n",
          bindings
        )
      )
      ...
      text =
        if weapon_name do
          Messages.interpolate(
            "但是$p淡淡一笑，浑然不将$N玄妙的招数放在眼中，随意将之架开。\n",
            bindings
          )
        else
