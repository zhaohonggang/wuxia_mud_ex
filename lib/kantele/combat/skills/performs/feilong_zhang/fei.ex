defmodule Kantele.Combat.Skills.Performs.FeilongZhang.Fei do
  @moduledoc """
  龙飞势「fei」（对照 `kungfu/skill/feilong-zhang/fei.c`）

  飞龙杖法攻击绝招：消耗 500 内力，发起 `combat/perform-incoming` 事件，
  由目标侧 `resolve_incoming` 判定命中并施加伤害。

  差异（TODO(migrate)）：
  - LPC 仅 `level:feilong-zhang>=160,force>=240`；本版加 `mapped` 与 `neili` 门槛；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "feilong-zhang")

    if level < 160 do
      {:error, "你飞龙杖法不够娴熟，难以施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "staff") == "feilong-zhang" do
      :ok
    else
      {:error, "你没有激发飞龙杖法，难以施展#{@move_name}。\n"}
    end
  end
  ...
  defp apply_perform(conn, character, target, level) do
    ...
    conn =
      Broadcast.publish(
        conn,
        "$N嗔目大喝，将全身劲力运于右臂，掌中兵刃顿时离手射出，施展#{@move_name}朝$n飞去！\n",
        n1: character.name,
        n2: target.name
      )
    ...
    if hit do
      ...
      |> Broadcast.publish(
        Messages.interpolate(
          "结果$p被$N的#{@move_name}击中胸口，撞断数根肋骨，受到#{damage}点伤害！\n",
          bindings
        )
      )
