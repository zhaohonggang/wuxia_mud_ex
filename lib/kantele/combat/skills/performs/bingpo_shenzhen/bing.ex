defmodule Kantele.Combat.Skills.Performs.BingpoShenzhen.Bing do
  @moduledoc """
  寒冰针「bing」（对照 `kungfu/skill/bingpo-shenzhen/bing.c`）

  冰魄神针单体攻击：消耗 150 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:bingpo-shenzhen>=120,force>=150`；本版加 `mapped` 与 `neili` 门槛；
  ...
  @perform_id "bingpo-shenzhen/bing"
  @move_name "「寒冰针」"
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "bingpo-shenzhen")

    if level < 120 do
      {:error, "你的冰魄神针不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "throwing") == "bingpo-shenzhen" do
      :ok
    else
      {:error, "你没有激发冰魄神针，无法施展#{@move_name}。\n"}
    end
  end
