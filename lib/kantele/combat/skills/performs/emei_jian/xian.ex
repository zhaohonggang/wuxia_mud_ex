defmodule Kantele.Combat.Skills.Performs.EmeiJian.Xian do
  @moduledoc """
  佛光初现「xian」（对照 `kungfu/skill/emei-jian/xian.c`）

  峨眉剑法单体攻击：消耗 100 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:emei-jian>=60`；本版加 `mapped` 与 `neili` 门槛；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "emei-jian")

    if level < 60 do
      {:error, "你的峨眉剑法不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "sword") == "emei-jian" do
      :ok
    else
      {:error, "你没有激发峨眉剑法，无法施展#{@move_name}。\n"}
    end
  end
  ...
  defp apply_perform(conn, character, target, level) do
    ...
    conn =
      Broadcast.publish(
        conn,
        "$N手中长剑一颤，剑尖吐出一道青光，正是峨眉剑法「佛光初现」，直指$n！\n",
        n1: character.name,
        n2: target.name
      )
