defmodule Kantele.Combat.Skills.Performs.QianyeShou.Qianye do
  @moduledoc """
  千叶「qianye」（对照 `kungfu/skill/qianye-shou/qianye.c`）

  千叶手单体攻击：消耗 100 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `skill:qianye-shou<120`；本版加 `mapped` 与 `neili` 门槛；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "qianye-shou")

    if level >= 120 do
      {:error, "你的千叶手已臻大成，无需再施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "hand") == "qianye-shou" do
      :ok
    else
      {:error, "你没有激发千叶手，无法施展#{@move_name}。\n"}
    end
  end
