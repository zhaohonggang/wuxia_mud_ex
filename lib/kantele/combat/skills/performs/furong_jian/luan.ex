defmodule Kantele.Combat.Skills.Performs.FurongJian.Luan do
  @moduledoc """
  乱剑覆花「luan」（对照 `kungfu/skill/furong-jian/luan.c`）

  芙蓉剑法攻击绝招：消耗 100 内力，发起 `combat/perform-incoming` 事件，
  由目标侧 `resolve_incoming` 判定命中并施加伤害。

  差异（TODO(migrate)）：
  - LPC 仅 `level:furong-jian>=100,force>=120,dodge>=120`；本版加 `mapped` 与 `neili` 门槛；
  - 命中判定：`rand(level) > parry/2`，忙乱 `level/22+2` 轮。
  - 伤害计算：`damage = level * 2`（简化），LPC 为 `level+rand(level/3)`。
  """
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "furong-jian")

    if level < 100 do
      {:error, "你芙蓉剑法不够娴熟，难以施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "sword") == "furong-jian" do
      :ok
    else
      {:error, "你没有激发芙蓉剑法，难以施展#{@move_name}。\n"}
    end
  end
  ...
  defp apply_perform(conn, character, target, level) do
    ...
    conn =
      Broadcast.publish(
        conn,
        "$N轻啸一声，腾空而起，手中兵刃在空中挽出一个剑花，施展#{@move_name}洒向$n全身！\n",
        n1: character.name,
        n2: target.name
      )
    ...
    if hit do
      ...
      |> Broadcast.publish(
        Messages.interpolate(
          "结果$p被$N的#{@move_name}刺得皮开肉绽，受到#{damage}点伤害！\n",
          bindings
        )
      )
