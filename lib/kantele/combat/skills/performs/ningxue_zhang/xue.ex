defmodule Kantele.Combat.Skills.Performs.NingxueZhang.Xue do
  @moduledoc """
  缤纷雪舞「xue」（对照 `kungfu/skill/ningxue-zhang/xue.c`）

  凝血掌单体攻击：消耗 200 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:ningxue-zhang>=50`；本版加 `mapped` 与 `neili` 门槛；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "ningxue-zhang")

    if level < 50 do
      {:error, "你的凝血掌法不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "staff") == "ningxue-zhang" do
      :ok
    else
      {:error, "你没有激发凝血掌法，无法施展#{@move_name}。\n"}
    end
  end
