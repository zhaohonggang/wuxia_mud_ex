defmodule Kantele.Combat.Skills.Performs.QiulinShiye.Wu do
  @moduledoc """
  五「wu」（对照 `kungfu/skill/qiulin-shiye/wu.c`）

  秋林师业单体攻击：无内力消耗，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 无门槛；本版加 `mapped` 门槛；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "qiulin-shiye")

    {:ok, level}
  end

  defp check_mapped(stats) do
    if Stats.mapped(stats, "unarmed") == "qiulin-shiye" do
      :ok
    else
      {:error, "你没有激发秋林师业，无法施展#{@move_name}。\n"}
    end
  end
  ...
  def resolve_incoming(conn, character, attacker, data) do
    ...
    bindings = [n1: attacker.name, n2: character.name, weapon2: weapon_name || "空手"]
