defmodule Kantele.Combat.Skills.Performs.ShangjiaDao.Juan do
  @moduledoc """
  寒芒暴卷「juan」（对照 `kungfu/skill/shangjia-dao/juan.c`）

  尚家刀法单体攻击：消耗 100 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:shangjia-dao>=50,force>=80`；本版加 `mapped` 与 `neili` 门槛；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "shangjia-dao")

    if level < 50 do
      {:error, "你的尚家刀法不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "blade") == "shangjia-dao" do
      :ok
    else
      {:error, "你没有激发尚家刀法，无法施展#{@move_name}。\n"}
    end
  end
