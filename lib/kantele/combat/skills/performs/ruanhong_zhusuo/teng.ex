defmodule Kantele.Combat.Skills.Performs.RuanhongZhusuo.Teng do
  @moduledoc """
  腾蛇诀「teng」（对照 `kungfu/skill/ruanhong-zhusuo/teng.c`）

  软红爪锁单体攻击：消耗 200 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:ruanhong-zhusuo>=60`；本版加 `mapped` 与 `neili` 门槛；
  ...
  @perform_id "ruanhong-zhusuo/teng"
  @move_name "「腾蛇诀」"
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "ruanhong-zhusuo")

    if level < 60 do
      {:error, "你的软红爪锁不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "whip") == "ruanhong-zhusuo" do
      :ok
    else
      {:error, "你没有激发软红爪锁，无法施展#{@move_name}。\n"}
    end
  end
