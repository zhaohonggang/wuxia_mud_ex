defmodule Kantele.Combat.Skills.Performs.SuxinJian.Lian do
  @moduledoc """
  连环剑术「lian」（对照 `kungfu/skill/suxin-jian/lian.c`）

  素心剑法单体攻击：消耗 200 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:suxin-jian>=120,dodge>=100`；本版加 `mapped` 与 `neili` 门槛；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "suxin-jian")

    if level < 120 do
      {:error, "你的素心剑法不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "sword") == "suxin-jian" do
      :ok
    else
      {:error, "你没有激发素心剑法，无法施展#{@move_name}。\n"}
    end
  end
