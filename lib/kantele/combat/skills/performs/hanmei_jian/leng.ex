defmodule Kantele.Combat.Skills.Performs.HanmeiJian.Leng do
  @moduledoc """
  冷剑决「leng」（对照 `kungfu/skill/hanmei-jian/leng.c`）

  寒梅剑法单体攻击：消耗 50 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:hanmei-jian>=40,force>=50`；本版加 `mapped` 与 `neili` 门槛；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "hanmei-jian")

    if level < 40 do
      {:error, "你的寒梅剑法不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "sword") == "hanmei-jian" do
      :ok
    else
      {:error, "你没有激发寒梅剑法，无法施展#{@move_name}。\n"}
    end
  end
