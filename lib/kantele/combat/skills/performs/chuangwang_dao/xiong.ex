defmodule Kantele.Combat.Skills.Performs.ChuangwangDao.Xiong do
  @moduledoc """
  雄霸群山「xiong」（对照 `kungfu/skill/chuangwang-dao/xiong.c`）

  创王刀法单体攻击：消耗 50 内力，对战斗中对手出招，命中由目标端
  `resolve_incoming` 判定并施加忙乱。

  差异（TODO(migrate)）：
  - LPC 仅 `level:chuangwang-dao>=40,force>=60`；本版加 `mapped` 与 `neili` 门槛；
  - 命中判定简化为 `rand(level) > parry/2`，忙乱轮数 `level/22+2`（同华山截手式）。
  """
  ...
  @perform_id "chuangwang-dao/xiong"
  @move_name "「雄霸群山」"
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "chuangwang-dao")

    if level < 40 do
      {:error, "你的创王刀法不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "blade") == "chuangwang-dao" do
      :ok
    else
      {:error, "你没有激发创王刀法，无法施展#{@move_name}。\n"}
    end
  end
  ...
  defp apply_perform(conn, character, target, level) do
    ...
    conn =
      Broadcast.publish(
        conn,
        "$N手中刀光暴涨，使出创王刀法绝技「雄霸群山」，一刀劈向$n，气势磅礴！\n",
        n1: character.name,
        n2: target.name
      )
