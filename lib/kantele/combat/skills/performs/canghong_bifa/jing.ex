defmodule Kantele.Combat.Skills.Performs.CanghongBifa.Jing do
  @moduledoc """
  惊虹一笔「jing」（对照 `kungfu/skill/canghong-bifa/jing.c`）

  沧虹笔法攻击绝招：消耗 500 内力，发起 `combat/perform-incoming` 事件。

  差异（TODO(migrate)）：
  - LPC 仅 `level:canghong-bifa>=100,force>=150`；本版加 `mapped` 与 `neili` 门槛；
  ...
  defp check_weapon(combat) do
    ...
    if weapon && Map.get(weapon, :skill_type) == "dagger" do
      :ok
    else
      {:error, "你使用的武器不对。\n"}
    end
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "canghong-bifa")

    if level < 100 do
      {:error, "你的沧虹笔法不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "dagger") == "canghong-bifa" do
      :ok
    else
      {:error, "你没有激发沧虹笔法，无法施展#{@move_name}。\n"}
    end
  end
