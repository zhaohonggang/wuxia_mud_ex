defmodule Kantele.Combat.Skills.Performs.MantianXing.Shan do
  @moduledoc """
  星光闪烁「shan」（对照 `kungfu/skill/mantian-xing/shan.c`）

  满天星单体攻击：消耗 100 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:mantian-xing>=100`；本版加 `mapped` 与 `neili` 门槛；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "mantian-xing")

    if level < 100 do
      {:error, "你的满天星不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "throwing") == "mantian-xing" do
      :ok
    else
      {:error, "你没有激发满天星，无法施展#{@move_name}。\n"}
    end
  end
  ...
  defp apply_perform(conn, character, target, level) do
    ...
    conn =
      Broadcast.publish(
        conn,
        "$N手中暗器如星光闪烁，使出满天星「星光闪烁」，暗器如流星般射向$n！\n",
        n1: character.name,
        n2: target.name
      )
