defmodule Kantele.Combat.Skills.Performs.XiyangJian.Ci do
  @moduledoc """
  刺「ci」（对照 `kungfu/skill/xiyang-jian/ci.c`）

  夕阳剑法单体攻击：消耗 300 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:xiyang-jian>=70,force>=120`；本版加 `mapped` 与 `neili` 门槛；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "xiyang-jian")

    if level < 70 do
      {:error, "你的夕阳剑法不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "sword") == "xiyang-jian" do
      :ok
    else
      {:error, "你没有激发夕阳剑法，无法施展#{@move_name}。\n"}
    end
  end
