defmodule Kantele.Combat.Skills.Performs.HanxingBada.Lastrisk do
  @moduledoc """
  孤注一掷「lastrisk」（对照 `kungfu/skill/hanxing-bada/lastrisk.c`）

  寒星八打单体攻击：消耗 100 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:hanxing-bada>=60`；本版加 `mapped` 与 `neili` 门槛；
  ...
  @perform_id "hanxing-bada/lastrisk"
  @move_name "「孤注一掷」"
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "hanxing-bada")

    if level < 60 do
      {:error, "你的寒星八打不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "throwing") == "hanxing-bada" do
      :ok
    else
      {:error, "你没有激发寒星八打，无法施展#{@move_name}。\n"}
    end
  end
