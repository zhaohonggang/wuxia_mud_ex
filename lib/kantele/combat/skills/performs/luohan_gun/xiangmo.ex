defmodule Kantele.Combat.Skills.Performs.LuohanGun.Xiangmo do
  @moduledoc """
  降魔「xiangmo」（对照 `kungfu/skill/luohan-gun/xiangmo.c`）

  罗汉棍法单体攻击：消耗 150 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:luohan-gun>=80`；本版加 `mapped` 与 `neili` 门槛；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "luohan-gun")

    if level < 80 do
      {:error, "你的罗汉棍法不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "club") == "luohan-gun" do
      :ok
    else
      {:error, "你没有激发罗汉棍法，无法施展#{@move_name}。\n"}
    end
  end
