defmodule Kantele.Combat.Skills.Performs.QingliangJian.Ming do
  @moduledoc """
  禅心自明「ming」（对照 `kungfu/skill/qingliang-jian/ming.c`）

  清凉剑法单体攻击：消耗 80 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:qingliang-jian>=80,force>=100`；本版加 `mapped` 与 `neili` 门槛；
  ...
  @perform_id "qingliang-jian/ming"
  @move_name "「禅心自明」"
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "qingliang-jian")

    if level < 80 do
      {:error, "你的清凉剑法不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "sword") == "qingliang-jian" do
      :ok
    else
      {:error, "你没有激发清凉剑法，无法施展#{@move_name}。\n"}
    end
  end

  defp check_neili(character) do
    if character.meta.vitals.neili < 80 do
