defmodule Kantele.Combat.Skills.Performs.LingsheZhangfa.Chan do
  @moduledoc """
  缠「chan」（对照 `kungfu/skill/lingshe-zhangfa/chan.c`）

  灵蛇杖法单体攻击：消耗 100 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:force>=80,staff>=80`；本版加 `mapped` 与 `neili` 门槛；
  ...
  @perform_id "lingshe-zhangfa/chan"
  @move_name "「缠」"
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "lingshe-zhangfa")

    if level < 80 do
      {:error, "你的灵蛇杖法不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "staff") == "lingshe-zhangfa" do
      :ok
    else
      {:error, "你没有激发灵蛇杖法，无法施展#{@move_name}。\n"}
    end
  end
