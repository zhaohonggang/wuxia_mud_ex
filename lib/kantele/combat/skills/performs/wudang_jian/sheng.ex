defmodule Kantele.Combat.Skills.Performs.WudangJian.Sheng do
  @moduledoc """
  太极初升「sheng」（对照 `kungfu/skill/wudang-jian/sheng.c`）

  武当剑法单体攻击：消耗 50 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:wudang-jian>=60,force>=80`；本版加 `mapped` 与 `neili` 门槛；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "wudang-jian")

    if level < 60 do
      {:error, "你的武当剑法不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "sword") == "wudang-jian" do
      :ok
    else
      {:error, "你没有激发武当剑法，无法施展#{@move_name}。\n"}
    end
  end
  ...
  defp apply_perform(conn, character, target, level) do
    ...
    conn =
      Broadcast.publish(
        conn,
        "$N剑势缓缓展开，使出武当剑法「太极初升」，剑气如旭日东升般涌向$n！\n",
        n1: character.name,
        n2: target.name
      )
