defmodule Kantele.Combat.Skills.Performs.YunheJian.Chong do
  @moduledoc """
  鹤冲九霄「chong」（对照 `kungfu/skill/yunhe-jian/chong.c`）

  云鹤剑法单体攻击：消耗 100 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:yunhe-jian>=100,force>=150`；本版加 `mapped` 与 `neili` 门槛；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "yunhe-jian")

    if level < 100 do
      {:error, "你的云鹤剑法不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "sword") == "yunhe-jian" do
      :ok
    else
      {:error, "你没有激发云鹤剑法，无法施展#{@move_name}。\n"}
    end
  end
  ...
  defp apply_perform(conn, character, target, level) do
    ...
    conn =
      Broadcast.publish(
        conn,
        "$N身形拔地而起，使出云鹤剑法「鹤冲九霄」，剑势如白鹤冲天般刺向$n！\n",
        n1: character.name,
        n2: target.name
      )
