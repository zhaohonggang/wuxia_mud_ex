defmodule Kantele.Combat.Skills.Performs.BaishengDaofa.Heng do
  @moduledoc """
  横扫千军「heng」（对照 `kungfu/skill/baisheng-daofa/heng.c`）

  百胜刀法攻击绝招：消耗 500 内力，发起 `combat/perform-incoming` 事件，
  由目标侧 `resolve_incoming` 判定命中并施加伤害。

  差异（TODO(migrate)）：
  - LPC 仅 `level:baisheng-daofa>=150,force>=200`；本版加 `mapped` 与 `neili` 门槛；
  ...
  @perform_id "baisheng-daofa/heng"
  @move_name "「横扫千军」"
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "baisheng-daofa")

    if level < 150 do
      {:error, "你的百胜刀法不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "blade") == "baisheng-daofa" do
      :ok
    else
      {:error, "你没有激发百胜刀法，无法施展#{@move_name}。\n"}
    end
  end
