defmodule Kantele.Combat.Skills.Performs.DuanyunFu.Tiaoyan do
  @moduledoc """
  挑燕「tiaoyan」（对照 `kungfu/skill/duanyun-fu/tiaoyan.c`）

  断云拂法单体攻击：消耗 100 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:duanyun-fu>=60`；本版加 `mapped` 与 `neili` 门槛；
  - 命中判定同华山截手式：`rand(level) > parry/2`，忙乱 `level/22+2` 轮。
  """
  ...
  defp check_weapon(combat) do
    ...
    if weapon && Map.get(weapon, :skill_type) == "club" do
      :ok
    else
      {:error, "你使用的武器不对。\n"}
    end
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "duanyun-fu")

    if level < 60 do
      {:error, "你的断云拂法不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "club") == "duanyun-fu" do
      :ok
    else
      {:error, "你没有激发断云拂法，无法施展#{@move_name}。\n"}
    end
  end
  ...
  defp apply_perform(conn, character, target, level) do
    ...
    conn =
      Broadcast.publish(
        conn,
        "$N手中拂尘轻挥，使出断云拂法「挑燕」，拂尘如灵蛇般刺向$n要害！\n",
        n1: character.name,
        n2: target.name
      )
