defmodule Kantele.Combat.Skills.Performs.LuoyunDao.Tian do
  @moduledoc """
  天刀落云式「tian」（对照 `kungfu/skill/luoyun-dao/tian.c`）

  洛云刀法单体攻击：消耗 300 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:luoyun-dao>=100`；本版加 `mapped` 与 `neili` 门槛；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "luoyun-dao")

    if level < 100 do
      {:error, "你的洛云刀法不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "blade") == "luoyun-dao" do
      :ok
    else
      {:error, "你没有激发洛云刀法，无法施展#{@move_name}。\n"}
    end
  end
