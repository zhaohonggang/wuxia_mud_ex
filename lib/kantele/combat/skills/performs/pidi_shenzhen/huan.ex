defmodule Kantele.Combat.Skills.Performs.PidiShenzhen.Huan do
  @moduledoc """
  幻遁寒星「huan」（对照 `kungfu/skill/pidi-shenzhen/huan.c`）

  辟地神针单体攻击：消耗 150 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:pidi-shenzhen>=140,force>=200`；本版加 `mapped` 与 `neili` 门槛；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "pidi-shenzhen")

    if level < 140 do
      {:error, "你的辟地神针不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "throwing") == "pidi-shenzhen" do
      :ok
    else
      {:error, "你没有激发辟地神针，无法施展#{@move_name}。\n"}
    end
  end
