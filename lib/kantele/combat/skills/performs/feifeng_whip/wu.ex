defmodule Kantele.Combat.Skills.Performs.FeifengWhip.Wu do
  @moduledoc """
  飞凤翔舞「wu」（对照 `kungfu/skill/feifeng-whip/wu.c`）

  飞凤鞭法攻击绝招：消耗 200 内力，发起 `combat/perform-incoming` 事件，
  由目标侧 `resolve_incoming` 判定命中并施加伤害。

  差异（TODO(migrate)）：
  - LPC 命中 `rand(whip) > force/2`，伤害 `(force+whip)/8+rand`，命中后另追 3 击，
    本版为 `rand(level) > parry/2` 单次结算，忙乱 `level/22+2` 轮；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "feifeng-whip")

    if level < 60 do
      {:error, "你的段家剑法不够娴熟，不会使用#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "whip") == "feifeng-whip" do
      :ok
    else
      {:error, "你现在没有激发段家剑，不能使用#{@move_name}。\n"}
    end
  end

  defp check_neili(character) do
    if character.meta.vitals.neili < 200 do
      {:error, "你现在真气不够，不能使用#{@move_name}。\n"}
    else
      :ok
    end
  end
  ...
  defp apply_perform(conn, character, target, level) do
    ...
    conn =
      Broadcast.publish(
        conn,
        "$N身子一晃，手中的长鞭画了一个圈子，施展#{@move_name}缠向$n！\n",
        n1: character.name,
        n2: target.name
      )
  ...
  def resolve_incoming(conn, character, attacker, data) do
    ...
    bindings = [n1: attacker.name, n2: character.name, weapon2: weapon_name || "长鞭"]
    ...
    if hit do
      ...
      |> Broadcast.publish(
        Messages.interpolate(
          "结果$p被$N的#{@move_name}击中直，浑身一震，内息涌动，受到#{damage}点伤害！\n",
          bindings
        )
      )
