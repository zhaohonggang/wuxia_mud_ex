defmodule Kantele.Combat.Skills.Performs.SanhuaZhang.San do
  @moduledoc """
  三花「san」（对照 `kungfu/skill/sanhua-zhang/san.c`）

  三花掌法单体攻击：消耗 200 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:sanhua-zhang>=135,force>=180`；本版加 `mapped` 与 `neili` 门槛；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "sanhua-zhang")

    if level < 135 do
      {:error, "你的三花掌法不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "strike") == "sanhua-zhang" do
      :ok
    else
      {:error, "你没有激发三花掌法，无法施展#{@move_name}。\n"}
    end
  end
