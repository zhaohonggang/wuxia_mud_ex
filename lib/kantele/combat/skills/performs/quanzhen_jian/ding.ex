defmodule Kantele.Combat.Skills.Performs.QuanzhenJian.Ding do
  @moduledoc """
  定阳针「ding」（对照 `kungfu/skill/quanzhen-jian/ding.c`）

  全真剑法单体攻击：消耗 150 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:quanzhen-jian>=100,force>=150`；本版加 `mapped` 与 `neili` 门槛；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "quanzhen-jian")

    if level < 100 do
      {:error, "你的全真剑法不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "sword") == "quanzhen-jian" do
      :ok
    else
      {:error, "你没有激发全真剑法，无法施展#{@move_name}。\n"}
    end
  end
