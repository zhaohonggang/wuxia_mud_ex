defmodule Kantele.Combat.Skills.Performs.ChuanxinZhang.Zhui do
  @moduledoc """
  穿心锥「zhui」（对照 `kungfu/skill/chuanxin-zhang/zhui.c`）

  穿心掌法攻击绝招：消耗 200 内力，发起 `combat/perform-incoming` 事件。

  差异（TODO(migrate)）：
  - LPC 仅 `level:chuanxin-zhang>=100,force>=150`；本版加 `mapped` 与 `neili` 门槛；
  - 命中判定：`rand(level) > parry/2`，忙乱 `level/22+2` 轮。
  - 伤害计算：`damage = level * 2`（简化）。
  """
  ...
  @perform_id "chuanxin-zhang/zhui"
  @move_name "「穿心锥」"
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "chuanxin-zhang")

    if level < 100 do
      {:error, "你的穿心掌法不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "strike") == "chuanxin-zhang" do
      :ok
    else
      {:error, "你没有激发穿心掌法，无法施展#{@move_name}。\n"}
    end
  end
  ...
  def resolve_incoming(conn, character, attacker, data) do
    ...
    bindings = [n1: attacker.name, n2: character.name, weapon2: weapon_name || "双掌"]
