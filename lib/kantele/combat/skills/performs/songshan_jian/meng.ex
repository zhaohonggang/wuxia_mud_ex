defmodule Kantele.Combat.Skills.Performs.SongshanJian.Meng do
  @moduledoc """
  如梦如幻「meng」（对照 `kungfu/skill/songshan-jian/meng.c`）

  嵩山剑法单体攻击：消耗 120 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:songshan-jian>=40`；本版加 `mapped` 与 `neili` 门槛；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "songshan-jian")

    if level < 40 do
      {:error, "你的嵩山剑法不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "sword") == "songshan-jian" do
      :ok
    else
      {:error, "你没有激发嵩山剑法，无法施展#{@move_name}。\n"}
    end
  end
  ...
  defp apply_perform(conn, character, target, level) do
    vitals = %{character.meta.vitals | neili: character.meta.vitals.neili - 120}
    character = %{character | meta: Map.put(character.meta, :vitals, vitals)}

    conn =
      Broadcast.publish(
        conn,
        "$N剑光如梦，使出嵩山剑法「如梦如幻」，剑影幻化罩向$n！\n",
        n1: character.name,
        n2: target.name
      )
