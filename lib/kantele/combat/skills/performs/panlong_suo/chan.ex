defmodule Kantele.Combat.Skills.Performs.PanlongSuo.Chan do
  @moduledoc """
  跗骨缠魂「chan」（对照 `kungfu/skill/panlong-suo/chan.c`）

  盘龙索法单体攻击：消耗 100 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:panlong-suo>=120,force>=180`；本版加 `mapped` 与 `neili` 门槛；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "panlong-suo")

    if level < 120 do
      {:error, "你的盘龙索法不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "whip") == "panlong-suo" do
      :ok
    else
      {:error, "你没有激发盘龙索法，无法施展#{@move_name}。\n"}
    end
  end
