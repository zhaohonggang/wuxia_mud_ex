defmodule Kantele.Combat.Skills.Performs.DuguJiujian.Yi do
  @moduledoc """
  弘意式「yi」（对照 `kungfu/skill/dugu-jiujian/yi.c`）

  独孤九剑攻击绝招：发起 `combat/perform-incoming` 事件，
  由目标侧 `resolve_incoming` 判定命中并施加伤害。

  差异（TODO(migrate)）：
  - LPC 仅 `skill<90`；本版加 `mapped` 与 `neili` 门槛；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "dugu-jiujian")

    if level < 90 do
      {:error, "你的独孤九剑不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "sword") == "dugu-jiujian" do
      :ok
    else
      {:error, "你没有激发独孤九剑，无法施展#{@move_name}。\n"}
    end
  end
  ...
  defp apply_perform(conn, character, target, level) do
    ...
    conn =
      Broadcast.publish(
        conn,
        "$N剑意流转，使出独孤九剑「弘意式」，剑招看似无意却暗藏玄机刺向$n！\n",
        n1: character.name,
        n2: target.name
      )
