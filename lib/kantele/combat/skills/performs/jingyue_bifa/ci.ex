defmodule Kantele.Combat.Skills.Performs.JingyueBifa.Ci do
  @moduledoc """
  残月刺「ci」（对照 `kungfu/skill/jingyue-bifa/ci.c`）

  惊月笔法单体攻击：消耗 100 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:jingyue-bifa>=30`；本版加 `mapped` 与 `neili` 门槛；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "jingyue-bifa")

    if level < 30 do
      {:error, "你的惊月笔法不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "dagger") == "jingyue-bifa" do
      :ok
    else
      {:error, "你没有激发惊月笔法，无法施展#{@move_name}。\n"}
    end
  end
