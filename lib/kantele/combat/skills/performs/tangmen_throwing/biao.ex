defmodule Kantele.Combat.Skills.Performs.TangmenThrowing.Biao do
  @moduledoc """
  镖「biao」（对照 `kungfu/skill/tangmen-throwing/biao.c`）

  唐门暗器单体攻击：无内力消耗，对战斗中对手出招。
  ...
  @perform_id "tangmen-throwing/biao"
  @move_name "「镖」"
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "tangmen-throwing")

    {:ok, level}
  end

  defp check_mapped(stats) do
    if Stats.mapped(stats, "throwing") == "tangmen-throwing" do
      :ok
    else
      {:error, "你没有激发唐门暗器，无法施展#{@move_name}。\n"}
    end
  end
