defmodule Kantele.Combat.Skills.Performs.SunFinger.Dian do
  @moduledoc """
  点穴「dian」（对照 `kungfu/skill/sun-finger/dian.c`）

  孙指单体攻击：无内力消耗，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:sun-finger>=80`；本版加 `mapped` 门槛；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "sun-finger")

    if level < 80 do
      {:error, "你的孙指不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "finger") == "sun-finger" do
      :ok
    else
      {:error, "你没有激发孙指，无法施展#{@move_name}。\n"}
    end
  end
