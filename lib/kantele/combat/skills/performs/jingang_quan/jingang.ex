defmodule Kantele.Combat.Skills.Performs.JingangQuan.Jingang do
  @moduledoc """
  金刚不坏「jingang」（对照 `kungfu/skill/jingang-quan/jingang.c`）

  金刚拳单体攻击：消耗 500 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:jingang-quan>=150,force>=150`；本版加 `mapped` 与 `neili` 门槛；
  ...
  @perform_id "jingang-quan/jingang"
  @move_name "「金刚不坏」"
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "jingang-quan")

    if level < 150 do
      {:error, "你的金刚拳不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "cuff") == "jingang-quan" do
      :ok
    else
      {:error, "你没有激发金刚拳，无法施展#{@move_name}。\n"}
    end
  end
