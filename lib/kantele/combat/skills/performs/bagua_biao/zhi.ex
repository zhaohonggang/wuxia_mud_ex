defmodule Kantele.Combat.Skills.Performs.BaguaBiao.Zhi do
  @moduledoc """
  乾坤一掷「zhi」（对照 `kungfu/skill/bagua-biao/zhi.c`）

  八卦镖法单体攻击：消耗 100 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:bagua-biao>=120,force>=200`；本版加 `mapped` 与 `neili` 门槛；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "bagua-biao")

    if level < 120 do
      {:error, "你的八卦镖法不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "throwing") == "bagua-biao" do
      :ok
    else
      {:error, "你没有激发八卦镖法，无法施展#{@move_name}。\n"}
    end
  end
  ...
  defp apply_perform(conn, character, target, level) do
    ...
    conn =
      Broadcast.publish(
        conn,
        "$N手中镖光一闪，使出八卦镖法「乾坤一掷」，一镖划破长空射向$n！\n",
        n1: character.name,
        n2: target.name
      )
