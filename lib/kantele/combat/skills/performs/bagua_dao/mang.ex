defmodule Kantele.Combat.Skills.Performs.BaguaDao.Mang do
  @moduledoc """
  劈天神芒「mang」（对照 `kungfu/skill/bagua-dao/mang.c`）

  八卦刀法攻击绝招：消耗 150 内力，发起 `combat/perform-incoming` 事件，
  由目标侧 `resolve_incoming` 判定命中并施加伤害。

  差异（TODO(migrate)）：
  - LPC 仅 `level:bagua-dao>=140,force>=200,wai-bagua>=140`；本版加 `mapped` 与 `neili` 门槛；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "bagua-dao")

    if level < 140 do
      {:error, "你的八卦刀法不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "blade") == "bagua-dao" do
      :ok
    else
      {:error, "你没有激发八卦刀法，无法施展#{@move_name}。\n"}
    end
  end
