defmodule Kantele.Combat.Skills.Performs.PangenCuojiefu.Cuo do
  @moduledoc """
  错字诀「cuo」（对照 `kungfu/skill/pangen-cuojiefu/cuo.c`）

  盘根错节斧单体攻击：消耗 100 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:pangen-cuojiefu>=100,force>=120`；本版加 `mapped` 与 `neili` 门槛；
  ...
  @perform_id "pangen-cuojiefu/cuo"
  @move_name "「错字诀」"
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "pangen-cuojiefu")

    if level < 100 do
      {:error, "你的盘根错节斧不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "hammer") == "pangen-cuojiefu" do
      :ok
    else
      {:error, "你没有激发盘根错节斧，无法施展#{@move_name}。\n"}
    end
  end
