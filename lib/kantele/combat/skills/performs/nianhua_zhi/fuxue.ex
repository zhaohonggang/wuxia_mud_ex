defmodule Kantele.Combat.Skills.Performs.NianhuaZhi.Fuxue do
  @moduledoc """
  拈花指穴「fuxue」（对照 `kungfu/skill/nianhua-zhi/fuxue.c`）

  拈花指单体攻击：消耗 200 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:finger>=150,force>=150`；本版加 `mapped` 与 `neili` 门槛；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "nianhua-zhi")

    if level < 150 do
      {:error, "你的拈花指不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "finger") == "nianhua-zhi" do
      :ok
    else
      {:error, "你没有激发拈花指，无法施展#{@move_name}。\n"}
    end
  end
