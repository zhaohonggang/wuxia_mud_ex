defmodule Kantele.Combat.Skills.Performs.Sougu.Muyeyingyang do
  @moduledoc """
  牧野英扬「muyeyingyang」（对照 `kungfu/skill/sougu/muyeyingyang.c`）

  搜骨鹰爪功单体攻击：消耗 800 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:sougu>=150`；本版加 `mapped` 与 `neili` 门槛；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "sougu")

    if level < 150 do
      {:error, "你的搜骨鹰爪功不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "claw") == "sougu" do
      :ok
    else
      {:error, "你没有激发搜骨鹰爪功，无法施展#{@move_name}。\n"}
    end
  end
  ...
  def resolve_incoming(conn, character, attacker, data) do
    ...
    bindings = [n1: attacker.name, n2: character.name, weapon2: weapon_name || "爪影"]
