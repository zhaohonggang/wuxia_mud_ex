defmodule Kantele.Combat.Skills.Performs.LongxingJian.Kong do
  @moduledoc """
  金龙疾空「kong」（对照 `kungfu/skill/longxing-jian/kong.c`）

  龙形剑法单体攻击：消耗 250 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:longxing-jian>=160`；本版加 `mapped` 与 `neili` 门槛；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "longxing-jian")

    if level < 160 do
      {:error, "你的龙形剑法不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "sword") == "longxing-jian" do
      :ok
    else
      {:error, "你没有激发龙形剑法，无法施展#{@move_name}。\n"}
    end
  end
