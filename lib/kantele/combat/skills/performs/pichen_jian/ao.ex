defmodule Kantele.Combat.Skills.Performs.PichenJian.Ao do
  @moduledoc """
  傲尘剑「ao」（对照 `kungfu/skill/pichen-jian/ao.c`）

  辟尘剑法单体攻击：消耗 200 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:pichen-jian>=100`；本版加 `mapped` 与 `neili` 门槛；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "pichen-jian")

    if level < 100 do
      {:error, "你的辟尘剑法不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "sword") == "pichen-jian" do
      :ok
    else
      {:error, "你没有激发辟尘剑法，无法施展#{@move_name}。\n"}
    end
  end
