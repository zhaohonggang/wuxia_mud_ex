defmodule Kantele.Combat.Skills.Performs.RaozhiRoujian.Chuan do
  @moduledoc """
  柔月穿空「chuan」（对照 `kungfu/skill/raozhi-roujian/chuan.c`）

  柔指柔剑单体攻击：消耗 250 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:raozhi-roujian>=100,force>=150`；本版加 `mapped` 与 `neili` 门槛；
  ...
  @perform_id "raozhi-roujian/chuan"
  @move_name "「柔月穿空」"
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "raozhi-roujian")

    if level < 100 do
      {:error, "你的柔指柔剑不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "sword") == "raozhi-roujian" do
      :ok
    else
      {:error, "你没有激发柔指柔剑，无法施展#{@move_name}。\n"}
    end
  end
  ...
  defp apply_perform(conn, character, target, level) do
    ...
    conn =
      Broadcast.publish(
        conn,
        "$N剑势柔中带刚，使出柔指柔剑「柔月穿空」，剑气如柔丝般穿向$n！\n",
        n1: character.name,
        n2: target.name
      )
