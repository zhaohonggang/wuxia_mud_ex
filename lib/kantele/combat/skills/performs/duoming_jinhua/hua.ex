defmodule Kantele.Combat.Skills.Performs.DuomingJinhua.Hua do
  @moduledoc """
  天外飞花「hua」（对照 `kungfu/skill/duoming-jinhua/hua.c`）

  夺命金花单体攻击：消耗 150 内力，对战斗中对手出招。

  差异（TODO(migrate)）：
  - LPC 仅 `level:duoming-jinhua>=120,force>=150`；本版加 `mapped` 与 `neili` 门槛；
  ...
  defp check_skill_level(stats) do
    level = Stats.skill(stats, "duoming-jinhua")

    if level < 120 do
      {:error, "你的夺命金花不够娴熟，无法施展#{@move_name}。\n"}
    else
      {:ok, level}
    end
  end
  ...
  defp check_mapped(stats) do
    if Stats.mapped(stats, "throwing") == "duoming-jinhua" do
      :ok
    else
      {:error, "你没有激发夺命金花，无法施展#{@move_name}。\n"}
    end
  end
  ...
  defp apply_perform(conn, character, target, level) do
    ...
    conn =
      Broadcast.publish(
        conn,
        "$N扬手飞出一朵金花，使出夺命金花「天外飞花」，金花如流星般射向$n！\n",
        n1: character.name,
        n2: target.name
      )
