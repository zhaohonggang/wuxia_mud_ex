...
- **批量进度**：T1 批次 1–14 已实装并推送 + HEAD 回归修复（全量 2809 绿），批次 13 起招式武学
  ...
  批次 6（6 条基础攻击：`bagua-biao/zhi`、`chanhun-suo/suo`、
  ...
  批次 10（6 条基础攻击：`longxing-jian/kong`、`luoying-shenjian/sui`、
  `luoyun-dao/tian`、`mantian-xing/shan`、`mantianhuayu-zhen/zheng`、`mingwang-jian/ruo`）＋
  批次 11（6 条基础攻击：`mizong-houquan/zhou`、`nianhua-zhi/fuxue`、
  `ningxue-zhang/xue`、`pangen-cuojiefu/cuo`、`panlong-suo/chan`、`pidi-shenzhen/huan`）＋
  批次 12（6 条基础攻击：`miaojia-jian/qian`、`mingwang-jian/ruo`、
  `poguang-dao/bing`、`pomie-jinzhen/du`、`qiankun-jian/riyue`、`miaojia-jian/zhai`）＋
  批次 13（6 条基础攻击：`qiufeng-chenfa/chan`、`quanzhen-jian/chan`、
  `sanwu-shou/wei`、`songshan-jian/meng`、`suxin-jian/lian`、`taiji-jian/chan`）＋
  批次 14（6 条基础攻击：`qiulin-shiye/wu`、`sun-finger/dian`、
  `taiji-jian/zhuan`、`tangmen-throwing/biao`、`tiangang-zhi/ling`、`sougu/muyeyingyang`）已实装、
  沿用 `perform-incoming`/`resolve_incoming` 目标侧结算样板（32 sword、6 blade、2 club、3 staff、6 whip、1 strike、1 hand、6 throwing、2 cuff、1 finger、1 dagger、1 force、1 hammer）。
校对清单 `docs/kungfu-f4-migration-checklist.zh-CN.md` 同步至 **226/644**
   （T1 85/86、T2 26/193、T3 108/169、T5 1/184）；T1 仅剩 `beiming-shengong/shield`
