defmodule ExKantele.World.Room.Qiyuan do
  @moduledoc """
  对应原文件: lpc_example/room/room_qiyuan2.c (棋苑标准棋房, 22443B)

  迁移判定: B/C —— 棋局引擎(围棋+五子棋)为纯函数可完整落地；仅需底层
  Room 交互载体(座位/对弈状态/消息)，故单文件给出完整纯函数引擎 + 交互骨架。

  ⚠️ 修正: 原目录 signature 误标「奇缘」(随机事件)；实际为 棋苑(Go/Gomoku 棋房)。
  本模块为忠实移植的围棋/五子棋完整规则引擎。

  # 规则引擎(纯函数)
    - board 表示为 `%{pos => color}`，pos = x*bsize+y，仅存已落子
    - Game struct: cells/bsize/turn/status/handicap/jie/历史
  """
